...
#define YCF_UART_PORT		"/dev/ttyUSB3"
#define YCF_UART_PORT1		"/dev/ttyS3"
...
static void *thread_uart(void* arg) {
	...
	jmethodID javaCallback = (*env)->GetMethodID(env, javaClass,
			"onCardNativeCallback", "([B)V");
	if (javaCallback == NULL) {
		LOGE("Fail to find method onCardNativeCallback");
		return 0;
	}

	LOGE("UART thread start.....");
	while (is_thread_running) {
		if ((len = UartWorkCard(fd, tmp_buf)) > 0) {
...
JNIEXPORT jint JNICALL Java_uartJni_UartJniCard_UartCardWriteCmd(JNIEnv *env, jobject thiz, jbyteArray arr, jint len) {
	int tempLen = -1;
	//unsigned char data[32] = {0};
	unsigned char *buf = (*env)->GetByteArrayElements(env, arr, false);
	int i = 0;

	LOGI(" Uart write cmd. len = %d", len);
	tempLen = write(fd, buf, len);
	//LOGD(" Uart write: 0x%02x", buf[0]);
	for (i = 0; i < len; i++) {
		LOGI("cmd[%d] =0x%02x, ", i, buf[i]);
	}
...
JNIEXPORT void JNICALL Java_uartJni_UartJniCard_nativeCardInitilize(JNIEnv *env, jobject thiz)
{
	(*env)->GetJavaVM(env, &gJavaVM);
	gJavaObj = (*env)->NewGlobalRef(env, thiz);
}

JNIEXPORT jint JNICALL Java_uartJni_UartJniCard_BoardCardThreadStart(JNIEnv *env, jobject thiz) {
...
JNIEXPORT void JNICALL Java_uartJni_UartJniCard_NativeCardThreadStop(JNIEnv *env, jobject thiz)
{
	int kill_rc;
